...
cmd_pxe_serve() {
    local iso="" root="$PXE_ROOT_DEFAULT" base="" _a sanboot=0 menu=0
    while [[ $# -gt 0 ]]; do
        case "$1" in
        --iso)
    ...
    if ((menu)); then
        # A console menu instead of one frozen answers file per MAC.
        #
        # The per-MAC arming model is right for unattended rebuilds: a machine
        # either has answers waiting or it does not. It is wrong when you are
        # standing at the console trying distros, because every attempt means
        # re-arming a MAC on the server. This mode offers the choice at boot.
        #
        # Consent still holds, by a different route: the DEFAULT is to boot the
        # local disk, and it selects itself after 30 seconds. A machine that
        # netboots by accident with nobody watching carries on booting as it
        # would have. Installing requires a person to choose it.
        mkdir -p "$root/answers"
        local _m_id _m_distro _m_profile _m_label
        # distro:profile:label — the installer supports arch, centos, debian,
        # fedora, rhel, rocky and ubuntu.
        for _a in \
            "rocky:server:Rocky 10 server" \
            "debian:server:Debian 13 server" \
            "ubuntu:server:Ubuntu server" \
            "centos:server:CentOS Stream 10 server" \
            "fedora:desktop:Fedora desktop" \
            "fedora:server:Fedora server"; do
            _m_distro="${_a%%:*}"
            _m_profile="$(cut -d: -f2 <<<"$_a")"
            _m_label="${_a##*:}"
            _m_id="${_m_distro}-${_m_profile}"
            cat >"$root/answers/${_m_id}.env" <<ANS
# Generated by deploy.sh pxe-serve --menu for the "${_m_label}" entry.
KLDLOAD_DISTRO=${_m_distro}
KLDLOAD_PROFILE=${_m_profile}
# Empty = auto-pick, excluding the live medium and the seed disk.
KLDLOAD_DISK=
KLDLOAD_HOSTNAME=kldload-${_m_distro}
KLDLOAD_USERNAME=admin
KLDLOAD_PASSWORD=Passw0rd
KLDLOAD_TIMEZONE=America/Los_Angeles
KLDLOAD_LOCALE=en_US.UTF-8
KLDLOAD_ENABLE_ZFS=1
# Off: MOK enrolment needs a human at the console and there is no unattended
# path through MokManager.
KLDLOAD_ENABLE_SECURE_BOOT=0
# Off for a menu install: golden images add about an hour of firstboot.
KLDLOAD_BUILD_IMAGES=0
ANS
        done
        log "  menu: wrote $(ls -1 "$root"/answers/*-*.env 2>/dev/null | wc -l) selectable answers files"

        {
            ...
            printf 'item --gap Install (wipes the disk):\n'
            for _a in \
                "rocky:server:Rocky 10 server" \
                "debian:server:Debian 13 server" \
                "ubuntu:server:Ubuntu server" \
            ...
            printf 'choose --timeout 30000 --default local target || goto local\n'
            printf 'goto ${target}\n\n'
            for _a in \
                "rocky:server" "debian:server" "ubuntu:server" \
                "centos:server" "fedora:desktop" "fedora:server"; do
                _m_id="${_a%%:*}-${_a##*:}"
                printf ':%s\n' "$_m_id"
                printf 'echo kldload: installing %s — this WIPES the disk\n' "$_m_id"
                printf 'kernel ${base}/kldload/vmlinuz initrd=initrd.img root=live:${base}/kldload/squashfs.img %s ip=dhcp rd.neednet=1 kldload.seed=${base}/answers/%s.env console=tty0\n' "${_iso_args}" "$_m_id"
        ...
        log "  menu: boot.ipxe written (default is the LOCAL disk after 30s)"
    elif ((sanboot)); then
        ...
        log "  sanboot: ISO linked at $root/iso/kldload.iso"
...
sanhook \${base}/seeds/\${seedimg} || goto notarmed
