// wgx — PROTOTYPE 0.1 of the WireGuard networks console (working name).
//
...
func usage() {
	fmt.Print(`wgx — WireGuard networks console (PROTOTYPE 0.1)

  wgx show                                     peer dossiers (root)
  ...
  wgx attach <net> <container> <member>        move iface into container netns (root)

Declarations: /etc/wgx/<name>.json — the file is the API.
`)
}
