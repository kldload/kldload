...
k_infra_calc_subnets() {
  ...
  for (( i = 0; i < 16; i++ )); do
    ...
    if (( i < 8 )); then KLDLOAD_SUBNET_BLUE+="${sub} "
    else               KLDLOAD_SUBNET_GREEN+="${sub} "
    fi
  done
...
guided_infra_prompt() {
  ...
  if [[ "${v}" == "yes" || "${v}" == "y" ]]; then
    ...
    KLDLOAD_WG0_NET="$(_ask  "wg0 network" "10.77.0.0/16")"; export KLDLOAD_WG0_NET
    KLDLOAD_WG0_PORT="$(_ask "wg0 port"    "51820")";        export KLDLOAD_WG0_PORT
    KLDLOAD_WG1_NET="$(_ask  "wg1 network" "10.78.0.0/16")"; export KLDLOAD_WG1_NET
    KLDLOAD_WG1_PORT="$(_ask "wg1 port"    "51821")";        export KLDLOAD_WG1_PORT
    KLDLOAD_WG2_NET="$(_ask  "wg2 network" "10.79.0.0/16")"; export KLDLOAD_WG2_NET
    KLDLOAD_WG2_PORT="$(_ask "wg2 port"    "51822")";        export KLDLOAD_WG2_PORT
    KLDLOAD_WG3_NET="$(_ask  "wg3 network" "10.80.0.0/16")"; export KLDLOAD_WG3_NET
    KLDLOAD_WG3_PORT="$(_ask "wg3 port"    "51823")";        export KLDLOAD_WG3_PORT
  fi
