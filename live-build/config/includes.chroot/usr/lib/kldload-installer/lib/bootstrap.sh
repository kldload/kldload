...
# zxplore ships by default (webui checkbox pre-checked; answerless
# autoinstalls get it too) — every image has ZFS, so every install gets its
# console unless explicitly unticked.
KLDLOAD_ENABLE_ZXPLORE=${KLDLOAD_ENABLE_ZXPLORE:-1}
...
# KVM: one answer, from k_kvm_wanted (common.sh). Recording anything else here
# is how the manifest came to say 1 while the package phase used 0.
...
k_bootstrap_base() {
    ...
    arch)
        _k_bootstrap_pacman
        ;;
    alpine | freebsd | openbsd | ghostbsd | illumos | windows)
        k_die "KLDLOAD_DISTRO=${distro} is not supported by this release (removed 2026-09-24 — see docs/release-1.0-manifest.md); supported: fedora rhel centos rocky debian ubuntu arch"
        ;;
    *)
...
# ══════════════════════════════════════════════════════════════════════════════
# Alpine, FreeBSD, OpenBSD, GhostBSD, illumos and Windows bootstraps lived
# here until 2026-09-24 (about 1,400 lines). None was offered by any menu,
# installed by any test, or documented; the release manifest
# (docs/release-1.0-manifest.md §7) cut them so the surface that ships is the
# surface the sweep proves. Git history has them if a maintainer appears.
# ══════════════════════════════════════════════════════════════════════════════
