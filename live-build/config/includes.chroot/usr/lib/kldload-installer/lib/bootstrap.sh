...
CUSTOMREPO
    ...
    # Fedora 44 kernel-7 lockout REINSTATED 2026-07-07. The future this hook
    # was left for has arrived: F44 updates jumped straight to kernel 7.1.3-200
    # (the 7.0.x line is gone), and OpenZFS 2.4.3 Conflicts: kernel-uname-r >
    # 7.0.999. Without the exclude, the target dnf pass resolves to 7.1.3,
    # zfs-dkms refuses to build, and the install drops to a dracut emergency —
    # the same class of break as the 2026-05-12 6.19.999 incident. The comment's
    # preferred "zfs bump" isn't available: the 2.4 repo still tops out at 2.4.3
    # and there's no 2.5 repo (404). So we re-populate the hook with a glob pin
    # (NOT a hardcoded 6.x/7.x version) — `kernel*-7.1*` excludes ONLY 7.1+ and
    # KEEPS 7.0.x, which zfs 2.4.3 supports (<=7.0.999). This is load-bearing at
    # install: the darksite ships kernel 7.0.14 — a broader `kernel*-7.*` filtered
    # it out and broke pass 3 (2026-07-07). Fedora-only: EL10 rides 6.12, never hits this.
    # Drop the exclude once a 7.1-capable OpenZFS lands. firstboot versionlock
    # then freezes whatever pair we installed here so `dnf update` can't drift.
    local _f44_kernel_lockout=()
    if [[ "${distro}" == "fedora" ]]; then
        _f44_kernel_lockout=('--exclude=kernel*-7.1*')
    fi
    ...
    dnf --installroot="${target}" --releasever="${release}" \
        ...
        >>"$log" 2>&1 ||
        {
            k_log_to "$log" "dnf --installroot pass 1 failed"
            return 1
        }
    ...
    dnf --installroot="${target}" --releasever="${release}" \
        ...
        >>"$log" 2>&1 ||
        {
            k_log_to "$log" "FATAL: dnf pass 3 (zfs + zfs-dkms + zfs-dracut) failed — target will not boot"
            return 1
        }
...
_k_bootstrap_freebsd() {
    ...
    if [[ ! -f "${darksite}/base.txz" ]]; then
        ...
        curl -fSL --connect-timeout 30 --max-time 600 --retry 3 \
            ...
            {
                k_log_to "$log" "FATAL: Failed to download FreeBSD base.txz from ${freebsd_url}"
                return 1
            }
    ...
    if [[ ! -f "${darksite}/kernel.txz" ]]; then
        ...
        curl -fSL --connect-timeout 30 --max-time 300 --retry 3 \
            ...
            {
                k_log_to "$log" "FATAL: Failed to download FreeBSD kernel.txz from ${freebsd_url}"
                return 1
            }
    ...
    [[ -f "${target}/bin/sh" ]] || {
        k_log_to "$log" "FATAL: FreeBSD base extraction failed — /bin/sh not found"
        return 1
    }
...
_k_bootstrap_openbsd() {
    ...
    curl -fSL --connect-timeout 30 --max-time 120 --retry 3 \
        ...
        {
            k_log_to "$log" "FATAL: Failed to download bsd.rd"
            return 1
        }
...
_k_bootstrap_windows() {
    ...
    if [[ "$win_iso" == *.iso ]]; then
        ...
        mount -o loop,ro "$win_iso" /mnt/win-iso 2>/dev/null ||
            {
                k_log_to "$log" "FATAL: Cannot mount Windows ISO"
                return 1
            }
    ...
    [[ -f "$wim_file" ]] || {
        k_log_to "$log" "FATAL: install.wim not found in ISO"
        return 1
    }
    ...
    wimapply "$wim_file" "$wim_index" /mnt/win-root >>"$log" 2>&1 ||
        {
            k_log_to "$log" "FATAL: wimapply failed"
            return 1
        }
