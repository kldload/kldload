...
bootstrap_write_manifest() {
    ...
    # The version comes from the ISO's own stamp, the same way
    # kldload-install-target derives it for its manifest: build-iso.sh writes
    # /etc/kldload/VERSION and the installer reads kldload_version out of it.
    # This used to be the literal "1.0.0", which sat here unchanged through
    # five releases. "unknown" rather than a plausible number when the file is
    # absent, so a missing stamp reads as missing.
    local _kld_iso_ver
    ...
    [[ -n "${_kld_iso_ver}" ]] || _kld_iso_ver="unknown"

    cat >"${manifest_dir}/install-manifest.env" <<EOF
# KLDload Install Manifest
# Generated by installer on $(date -u '+%Y-%m-%dT%H:%M:%SZ')

# Canonical "came-from-installer, not live-ISO" flag. kldload-webui's
# is_live_mode() reads this; without it, installed boxes keep rendering
# the pre-install installer UI forever.
KLDLOAD_INSTALLED=1
