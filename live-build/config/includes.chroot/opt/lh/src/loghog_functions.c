...
void find_logs_command(char *buffer, size_t size, const char *search_path) {
    // Builds the base find+tail command (no filters). search_path may
    // contain multiple space-separated directories — find walks them all.
    // No single-quoting around %s: quoting would force find to treat the
    // whole string as one path, breaking the multi-path contract the
    // default log_search_path relies on. Callers are expected to pass
    // trusted paths (menu input is sanitised by sanitize_input()).
    //
    // tail -n 2000 gives ~2000 lines of recent context per file before
    // the live stream starts. Big enough to capture the last 10-15 min
    // on a normal node; small enough to not overwhelm less on a 5-node
    // cluster-logs mount (~10k lines total at startup).
    //
    // Matchers now cover both RPM (*messages, secure, firewalld) and
    // Debian/Ubuntu (*syslog, auth.log, kern.log) log file names plus
    // auditd's audit.log.
    (void)snprintf(
        buffer, size,
        "find %s -type f \\( "
        "-name '*.log' -o -name 'messages' -o -name 'cron' -o -name 'maillog' "
        "-o -name 'secure' -o -name 'firewalld' -o -name 'kern.log' "
        "-o -name 'auth.log' -o -name 'syslog' -o -name 'audit.log' "
        "\\) 2>/dev/null -exec tail -n 2000 -f {} +",
        search_path
    );
}
...
void main_menu(void) {
    while (1) {
        // Clean minimal menu — no ASCII-art banner, no per-letter rainbow.
        // Single dim header, one option per line, single-letter prompt.
        // Keeps the fast feel without the visual noise.
        printf("\nlh — log hog\n");
        printf("  a   auth errors\n");
        printf("  e   errors / warnings\n");
        printf("  l   live tail — all logs\n");
        printf("  n   network protocols\n");
        printf("  r   regex search\n");
        printf("  i   ip / regex search\n");
        printf("  s   set log paths\n");
        printf("  j   export results → json\n");
        printf("  h   help\n");
        printf("  q   quit\n");
        printf("\n> ");
        ...
        switch (opt) {
            ...
            default: printf("unknown — pick one of a/e/l/n/r/i/s/j/h/q\n"); break;
        }
        free(option);
    }
}
