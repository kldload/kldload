...
# ── Prompt ────────────────────────────────────────────────────────────────────
# Compact k8s indicator for the prompt. Emits a short "k8s:<namespace> " ONLY
# when there's a real current-context — the label/space live INSIDE the output,
# so a non-k8s shell (sysdiag / local console) gets a clean prompt with no empty
# "[]" and no clutter. Works with or without kubernetes: no kubectl, no context,
# or an empty context all short-circuit to a plain prompt.
# HISTORY: "[kubernetes-admin@kubernetes/default]" on every prompt buried a local
# root shell in cluster noise (operator on .104); then the U+2388 helm glyph
# tofu-boxed on the default DejaVu Sans Mono font (onyx 2026-07-05). ASCII "k8s:"
# renders in every font on every distro.
__ps1_k8s_ns() {
  ...
  # \001/\002 (readline non-printing markers), NOT \[ \]: this text is emitted
  # from a $() in PS1, where bash's \[ \] escapes are not reprocessed.
  printf '\001\e[96m\002k8s:%s \001\e[0m\002' "${ns:-default}"
}
