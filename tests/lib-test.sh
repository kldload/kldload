...
test_cmd() {
  local name="$1" cmd="$2"
  if command -v "$cmd" >/dev/null 2>&1; then _pass "$name"
...
test_service_active() {
  ...
  if [[ "$state" == "active" ]]; then _pass "$name"
...
test_service_enabled() {
  ...
  if [[ "$state" == "enabled" ]]; then _pass "$name"
...
test_dataset() {
  local name="$1" ds="$2"
  if zfs list "$ds" >/dev/null 2>&1; then _pass "$name"
...
test_output_contains() {
  ...
  local output
  output=$(eval "$cmd" 2>&1)
  if echo "$output" | grep -qi "$expected"; then _pass "$name"
...
test_succeeds() {
  local name="$1" cmd="$2"
  if eval "$cmd" >/dev/null 2>&1; then _pass "$name"
